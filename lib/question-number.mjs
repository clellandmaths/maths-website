...
/**
 * The whole printed label — "2023 P1 Q8" — or null when there is no badge.
 *
 * The same anchor as `questionNumber()`, and for the same reason: the second
 * white-space:nowrap span in a question is a measurement, not a label.
 *
 * This is the string the generator's variation registry uses in `basedOn`,
 * character for character, so it is what "more questions like this one" is
 * keyed on. **Read it here rather than rebuilding it** from year, paper number
 * and question number: `lib/specials-loader.ts` sets `questionNumber` to a
 * question's position in a Marathon rather than its printed number, so a
 * rebuilt label would be well-formed, plausible, and point at a different
 * question. Nothing would throw.
 *
 * @param {string} questionHtml
 * @returns {string | null}
 */
export function paperRef(questionHtml) {
