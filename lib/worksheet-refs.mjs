 ...
 *     _xqt6z3f9a1    a generated one - the mark, 5-char code, 6-char seed
 *
...
export const GENERATED_TOKEN_LENGTH = 1 + CODE_LENGTH + SEED_LENGTH;
 ...
 * drift silently.
 */
export function generatedRef(code, seed) {
  return `g:${code}:${seed}`;
}

/** "g:xqt6z:3f9a1b" split back into its parts, or null if it is not one. */
export function parseGeneratedRef(ref) {
  if (typeof ref !== 'string') return null;
  const parts = ref.split(':');
  if (parts.length !== 3 || parts[0] !== 'g') return null;
  const [, code, seed] = parts;
  if (!BASE36.test(code) || !BASE36.test(seed)) return null;
  return { code, seed };
}

/** Twelve characters for one generated question, or null if it will not fit. */
export function packGenerated(code, seed) {
  ...
  return GENERATED_MARK + code + seed;
}
...
export function unpackGenerated(token) {
  ...
  const seed = token.slice(1 + CODE_LENGTH);
  if (!BASE36.test(code) || !BASE36.test(seed)) return null;
  return generatedRef(code, seed);
}
