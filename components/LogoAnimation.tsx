...
// The arrow is ONE solid silhouette (shaft + barbed head) knocked out of the
// mint — its tip pokes just past the square's corner into the background,
// leaving mint bordering it on the remaining sides (not a separate mint
// triangle layered outside — that produced a broken sliver, since it sat
// inside the masked group and got cut by the neighbouring knockout strokes).
...
// Arrow: shaft from the circle's centre to the barb line, then a triangular
// head whose tip pokes just past the square's corner (686.5,14.5).
const ARROW_SHAFT = 'M 494.5 206.5 L 643 58';
const ARROW_HEAD = 'M 689 12 L 661 76 L 625 40 Z';
      ...
      <svg viewBox="0 0 836 536" fill="none" className="absolute inset-0 w-full h-full">
        <defs>
          <mask id="cm-mark">
            ...
            {/* arrow — shaft + solid barbed head, one knockout silhouette */}
            <path d={ARROW_SHAFT} stroke="black" strokeWidth="16" />
            <path d={ARROW_HEAD} fill="black" />
        ...
        <g className="geo-construction" style={{ ...anim(1.5), '--geo-len': 100 } as React.CSSProperties}>
          ...
          <path d={ARROW_SHAFT} stroke={MINT} strokeWidth="2.5" strokeLinecap="round" pathLength={100} className="geo-line" style={anim(0.8, 0.9)} />
          <path d={ARROW_HEAD} stroke={MINT} strokeWidth="2.5" strokeLinejoin="round" pathLength={100} className="geo-line" style={anim(0.95, 0.9)} />
